...
def add_word_to_db(user_id, english_word, russian_word):
    conn = connect_db()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO user_words (user_id, english_word, russian_word) VALUES (%s, %s, %s) ON CONFLICT (user_id, english_word) DO NOTHING", (user_id, english_word, russian_word))
    conn.commit()
    cursor.close()
    conn.close()
...
def get_words_from_db(user_id):
    conn = connect_db()
    cursor = conn.cursor()
    cursor.execute("SELECT english_word, russian_word FROM user_words WHERE user_id = %s", (user_id,))
...
def remove_word_from_db(user_id, english_word):
    conn = connect_db()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM user_words WHERE user_id = %s AND english_word = %s", (user_id, english_word))
    conn.commit()
    cursor.close()
    conn.close()
